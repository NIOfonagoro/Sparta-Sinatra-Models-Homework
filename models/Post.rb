class Post
  ...
  attr_accessor :id,  :last_name, :email
  ...
  def save
    ...
    if (!self.id)
      sql = "INSERT INTO post (last_name, email) VALUES ('#{ self.last_name}','#{ self.email }')"
    else
      sql = "UPDATE post SET last_name='#{self.last_name}', body='#{self.email}' WHERE id='#{self.id}'"
    end
